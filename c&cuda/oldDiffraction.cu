...
__global__ void zero_total(cPixel *cPixels, int num_cPixels)
{
	int pixIdx = threadIdx.x + blockIdx.x * blockDim.x;
	
	if(pixIdx < num_cPixels)
	{
		cPixels[pixIdx].total = 0;
	}
	__syncthreads();
}
__global__ void sum_diffraction(cPixel *cPixels, int num_cPixels)
{
	int pixIdx = threadIdx.x + blockIdx.x * blockDim.x;
	
	if(pixIdx < num_cPixels)
	{
		cPixels[pixIdx].total += cPixels[pixIdx].I;
	}
	__syncthreads();
}
__global__ void diffraction_event(cPixel *cPixels, cComplex *sf, int num_pix, cAtom *lattice, int num_cAtoms, char *Z, int numZ)
{
	...
	if(pixIdx < num_pix)
	{
		...
		for(int j = 0; j < numZ; j++) {
			...
			{
				if(lattice[i].Z != elemTypes[j]) { continue; }
