...
public class TestingINPFileAnalyzer {
	 ...
	 * @param aFile
	 */
	public TestingINPFileAnalyzer(String xyzORinp, File aFile)
	{/*
	...
	public Object[][][] calculateClosenessToAllFour(JVector[][] set)
	{
		int howManyOutside = 0;
		
		//int numBromines = (int) (Math.pow(aUnits / 2, 3) * 4 * 4); 
		
		// [JVector][Point2D.Double][Point2D.Double]
		Object[][][] xy = new Object[set.length][4][numBromines/4];
		Point2D.Double[] tempPoint = new Point2D.Double[4];
		Double[] tempVal = new Double[4];
		Double[][] fourAngles = new Double[4][numBromines/4];
		Double[] carbonAngles = new Double[numBromines/4];
		int i = 0;
		int angleIndex = 0;
		int whichVector = 0;
		int[][] indices = new int[set.length][4];
		for(int a = 0; a < lattice.length; a++)
		{
			JVector[] bromines = lattice[a].getVectors();
			// loop through the bromines
			for(int j = 1; j < bromines.length; j++)
			{
				...
				for(int k = 0; k < set.length; k++)
				{
					...
					try {
						// angle is in degrees
						angle = Math.acos(JVector.dot(bromines[j], set[k][0])) * 180 / Math.PI;
					} catch (Exception e) {
						System.out.println("angle fail in INP analyzer");
					}
					TwoNodeVector temp = new TwoNodeVector(angle, k, null, null);
				...
				fourAngles[j-1][angleIndex] = closest.getValue();
				
				whichVector = closest.getKey();
				
				// determine the x-coordinates
				double xCoord = 0;
				try {
					xCoord = JVector.dot(bromines[j], set[whichVector][1]);
					if(xCoord > 500) {
				...
				double yCoord = 0;
				try {
					yCoord = JVector.dot(bromines[j], set[whichVector][2]);
					if(yCoord > 500) {
				...
				tempVal[j-1] = fourAngles[j-1][angleIndex];
			...
			for(int j = 0; j < 4; j++) {
				...
				tempVal[min] = 1000.;
				xy[whichVector][j][indices[whichVector][j]++] = tempPoint[min].clone();
			}
			i++;
			angleIndex++;		
		...
		for(int k = 0; k < 4; k++) {
			for(int j = 0; j < fourAngles[k].length; j++) {
				ps.print(fourAngles[k][j] + "\t");
			}
			ps.println();
		}
		System.out.println(howManyOutside);
	...
	*/
	...
	public static void printFourAngles(String fName, Object[][][] coords, JVector[][] axes) {
		MyPrintStream mps = null, mpsTotal = new MyPrintStream(new File(fName+"_total"));
		// print each projection independently
		for(int k = 0; k < coords.length; k++) {
			// print each angle independently
			for(int j = 0; j < coords[k].length; j++) {
				mps = new MyPrintStream(new File(fName+"_"+axes[k][0].toString() + "_" + j));
				for(int i = 0; i < coords[k][j].length; i++) {
					if(coords[k][j][i] != null) {
						mps.println(((Point2D.Double) coords[k][j][i]).x + "\t" + ((Point2D.Double) coords[k][j][i]).y);
						mpsTotal.println(((Point2D.Double) coords[k][j][i]).x + "\t" + ((Point2D.Double) coords[k][j][i]).y);
					}
				}
				mps.close();
			}
	...
	public void getLatticesFromFileList(File[] lattices, int numToRead, String xyzorinp) {
		...
		int maxIdx = (int) Math.min(numToRead, lattices.length);
	...
	public static void main(String[] args)
	{
		...
		folder = new File("Z:\\Simulation\\Eric\\CBr4\\Dill-4\\126\\simulOut\\batch test");
		...
		TestingINPFileAnalyzer test = new TestingINPFileAnalyzer("inp", new File(path + "/outAfter.xyz"));
		test.aConstant = 8.82;
		...
		for(int i = 0; i < numProj; i++) {
			...
			printFourAngles(prefix+fName, test.calculateClosenessToAllFour(axes), axes);
		}
		FileOutputStream fos = null;
		try {
